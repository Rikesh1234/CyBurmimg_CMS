...
exports.createPost = async (req, res) => {
    try {
      ...
      const { title, slug, tag_line, summary, content, category, author, tags, photo_gallery, published, published_date } = req.body;
  
      // Handle featured image upload
      const featured_image = req.files['featured_image'] ? `/uploads/post/${req.files['featured_image'][0].filename}` : '/images/default.jpg';
  
      // Handle gallery images upload
      const gallery_images = req.files['gallery_images'] ? req.files['gallery_images'].map(file => `/uploads/post/gallery/${file.filename}`) : [];
...
exports.updatePost = async (req, res) => {
  const postId = req.params.postId;
  const { title, slug, tag_line, summary, content, category, author, tags, published, published_date } = req.body;
  ...
  if (!content || content.trim() === '') errorMessages.push('Content is required');
  if (!category || category.length === 0) errorMessages.push('At least one category is required');
  ...
  try {
    ...
    const gallery_images = req.files['gallery_images'] 
      ? req.files['gallery_images'].map(file => `/uploads/post/gallery/${file.filename}`) 
